...
if (store == NULL)
{
            ...
            column=gtk_tree_view_column_new_with_attributes ("Race",renderer,"text",DEPART,NULL);
            gtk_tree_view_append_column(GTK_TREE_VIEW(treeview3),column);

            renderer=gtk_cell_renderer_text_new ();
            column=gtk_tree_view_column_new_with_attributes ("Gender",renderer,"text",DESTINATION,NULL);
            gtk_tree_view_append_column(GTK_TREE_VIEW(treeview3),column);

            renderer=gtk_cell_renderer_text_new ();
            column=gtk_tree_view_column_new_with_attributes ("Birthday",renderer,"text",DATEDEPART,NULL);
            gtk_tree_view_append_column(GTK_TREE_VIEW(treeview3),column);

             renderer=gtk_cell_renderer_text_new ();
            column=gtk_tree_view_column_new_with_attributes ("Numbre",renderer,"text",DATERETOUR,NULL);
            gtk_tree_view_append_column(GTK_TREE_VIEW(treeview3),column);
}
