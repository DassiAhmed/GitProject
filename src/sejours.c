...
if (store == NULL)
{
            ...
            column=gtk_tree_view_column_new_with_attributes ("Plante",renderer,"text",HOTELL,NULL);
            gtk_tree_view_append_column(GTK_TREE_VIEW(treeview3),column);

            renderer=gtk_cell_renderer_text_new ();
            column=gtk_tree_view_column_new_with_attributes ("Numbre",renderer,"text",VILLEE,NULL);
            gtk_tree_view_append_column(GTK_TREE_VIEW(treeview3),column);

            renderer=gtk_cell_renderer_text_new ();
            column=gtk_tree_view_column_new_with_attributes ("Mois-favorable",renderer,"text",CATEGORIEE,NULL);
            gtk_tree_view_append_column(GTK_TREE_VIEW(treeview3),column);
}
